...
class Game:
  ...
  def game_loop(self):
    while not validation.validate_game_status(self.currentFen) and not self.stopGame:
      ...
      # Detecta o movimento do usuario pela diferença das bitBoards
          ...
          if board.is_move_to_eat_piece(move, self.currentBoard, self.currentFen):
              ...
              if self.userCastlingAvailable and board.is_castling(move, self.color, self.currentBoard):
              ...
              communication.update_status_message("Erro! Volte o tabuleiro para o estado anterior.")
              self.recover_mode()
          else:
  ...
  def user_castling_move(self, move):
      ...
      while True:
          communication.update_status_message(f"Mova a torre para completar o castling! {towerMove}")
  ...
  def AI_castling_move(self, move, message):
      towerMove = self.castling_move(move)
      self.AICastlingAvailable = False
      movements.game_movement(towerMove)
      communication.update_status_message(f"IA Casltling move concluído! {towerMove}")
  ...
  def make_AI_movement(self):
      ...
      # Castling logic
      if self.AICastlingAvailable and board.is_castling(bestMove, self.color, self.currentBoard):
